    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6">
      <div class="flex items-center justify-between border-b-2 border-gray-100 py-6 md:justify-start md:space-x-10">
        ...
        <PopoverGroup as="nav" class="hidden space-x-10 md:flex">
            ...
            <!-- Nav List Large  -->
          <a v-for="nav in mobileList" :key="nav.id" href="nav.href" class="text-base font-medium text-gray-900 hover:text-walter-primary">{{nav.name}}</a>
      ...
      <PopoverPanel focus class="absolute inset-x-0 top-0 origin-top-right transform p-2 transition md:hidden">
        <div class="divide-y-2 divide-gray-50 rounded-lg bg-white shadow-lg ring-1 ring-black ring-opacity-5">
          <div class="px-5 pt-5 pb-6">
            ...
            <div class="mt-6">
              <nav class="grid gap-y-8">
                <a v-for="item in mobileList" :key="item.name" :href="item.href" class="-m-3 flex items-center rounded-md p-3 hover:bg-gray-50">
                  <component :is="item.icon" class="h-6 w-6 flex-shrink-0 text-black" aria-hidden="true" />
          ...
          <div class="space-y-6 py-6 px-5">
            ...
            <div>
              <a href="#" class="flex w-full items-center justify-center rounded-md border border-transparent bg-walter-primary px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-black">Sign up</a>
              <p class="mt-6 text-center text-base font-medium text-gray-500">
                Existing customer?
                {{ ' ' }}
                <a href="#" class="text-black hover:text-walter-primary">Sign in</a>
              </p>
            </div>
          </div>
        </div>
...
export default{
    ...
    setup(){
        ...
        return {recentPosts, callsToAction, resources, solutions, mobileList}
    }
}
